...
{
	document.location.href = 'documentos.php';
}
</script>
</head>
...
if($_POST['datosok']=='si')
{
	if(!isset($_POST['iddocumento']) || $_POST['iddocumento'] == '')
	{
		...
		if(!$buscarins)
			print ("<script>alert('Error al ingresar el documento')</script>");
		else
			?><!--<script>alert ('el registro ha sido ingresado satisfactoriamente')</script>--><?	
	}
	elseif(isset($_POST['iddocumento']) && $_POST['iddocumento'] != '')
	{
		$queryins="UPDATE $tabla SET nombre=UCASE('$_POST[nombre]'), observaciones=UCASE('$_POST[observaciones]') WHERE $llave='$_POST[iddocumento]'";
		//print $queryins; 
		$buscarins=mysqli_query($link,$queryins);
		if(!$buscarins)
			print mysqli_error();
...
{
	$queryelim="DELETE FROM $tabla WHERE $llave='$_POST[iddocumento]'";
...
if (isset($_POST['iddocumento']))
	$_GET['iddocumento'] == $_POST['iddocumento'];
...
<input name="iddocumento" type="hidden" value="<? print $_GET['iddocumento'] ?>" />
<input name="xyz" type="hidden" value="" />

<?
	if($_GET['iddocumento'] != '')
	{
		$sqlad = "select * from $tabla where $llave=$_GET[iddocumento]";
...
if(isset($_POST['xyz']) and $_POST['xyz']!="")
{
	...
	if ($filas == 0)
		print ("<script>alert('Este documento no existe vuelva a intentarlo')</script>");
	else
	{
		if ($filas == 1)
		{
			$row = mysqli_fetch_array($query);
			print ("<script>document.location.href='".$_SERVER['PHP_SELF']."?iddocumento=$row[iddocumento]'</script>");
		}
		?>
		<table width="50%"align="center">
			<tr align="center">
				<td valign="top">
					...
					{
						print "<a href=".$_SERVER['PHP_SELF']."?iddocumento=$row[iddocumento]>$row[nombre]</a><br>";
					}
					?>
