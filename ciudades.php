...
{
	document.location.href = 'ciudades.php';
}
</script>
</head>
...
if($_POST['datosok']=='si')
{
	...
	{
		$queryins="INSERT INTO $tabla (idpais, nombre, observaciones) VALUES(UCASE('$_POST[idpais]'), UCASE('$_POST[nombre]'), UCASE('$_POST[observaciones]'))";
		//print $queryins; 
	...
	elseif(isset($_POST['idciudad']) && $_POST['idciudad'] != '')
	{
		$queryins="UPDATE $tabla SET idpais=UCASE('$_POST[idpais]'), nombre=UCASE('$_POST[nombre]'), observaciones=UCASE('$_POST[observaciones]') WHERE $llave='$_POST[idciudad]'";
		//print $queryins; 
		$buscarins=mysqli_query($link,$queryins);
...
{
	$queryelim="DELETE FROM $tabla WHERE $llave='$_POST[idciudad]'";
...
<input name="idciudad" type="hidden" value="<? print $_GET['idciudad'] ?>" />
<input name="xyz" type="hidden" value="" />

<?
	if($_GET['idciudad'] != '')
	{
		$sqlad = "select * from $tabla where $llave=$_GET[idciudad]";
...
if(isset($_POST['xyz']) and $_POST['xyz']!="")
{
	...
	else
	{
		...
		{
			$row = mysqli_fetch_array($query);
			print ("<script>document.location.href='".$_SERVER['PHP_SELF']."?idciudad=$row[idciudad]'</script>");
		}
		?>
		<table width="50%"align="center">
			<tr align="center">
				<td valign="top">
					...
					{
						print "<a href=".$_SERVER['PHP_SELF']."?idciudad=$row[idciudad]>$row[nombre]</a><br>";
					}
					?>
...
<table width="100%" align="center">
	...
	<tr>
    	...
    	<td>
        	<select id="idpais" name="idpais" class="tex1" >
                ...
                {
					if($_GET['idpais']!='')
						$comparador = $_GET['idpais'];
					else
						$comparador = $datosad['idpais'];	
					$sel = "";
