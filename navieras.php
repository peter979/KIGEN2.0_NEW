...
{
	document.location.href = 'navieras.php';
}
...
if($_POST['datosok']=='si')
{
	if(!isset($_POST['idnaviera']) || $_POST['idnaviera'] == '')
	{
		$queryins="INSERT INTO $tabla (nombre, num_contrato, observaciones) VALUES(UCASE('$_POST[nombre]'),  UCASE('$_POST[num_contrato]'), UCASE('$_POST[observaciones]'))";
		//print $queryins; 
		$buscarins=mysqli_query($link,$queryins);
			
		if(!$buscarins)
			print ("<script>alert('No se pudo insertar la naviera')</script>");
		else
			?><script>alert ('el registro ha sido ingresado satisfactoriamente')</script><?	
	}
	elseif(isset($_POST['idnaviera']) && $_POST['idnaviera'] != '')
	{
		$queryins="UPDATE $tabla SET nombre=UCASE('$_POST[nombre]'), num_contrato=UCASE('$_POST[num_contrato]'), observaciones=UCASE('$_POST[observaciones]') WHERE $llave='$_POST[idnaviera]'";
		//print $queryins; 
		$buscarins=mysqli_query($link,$queryins);
		if(!$buscarins)
			print mysqli_error();
...
{
	$queryelim="DELETE FROM $tabla WHERE $llave='$_POST[idnaviera]'";
...
if (isset($_POST['idnaviera']))
	$_GET['idnaviera'] == $_POST['idnaviera'];
...
<input name="idnaviera" type="hidden" value="<? print $_GET['idnaviera'] ?>" />
<input name="xyz" type="hidden" value="" />

<?
	if($_GET['idnaviera'] != '')
	{
		$sqlad = "select * from $tabla where $llave=$_GET[idnaviera]";
...
if(isset($_POST['xyz']) and $_POST['xyz']!="")
{
	...
	else
	{
		...
		{
			$row = mysqli_fetch_array($query);
			print ("<script>document.location.href='".$_SERVER['PHP_SELF']."?idnaviera=$row[$llave]'</script>");
		}
		?>
		<table width="50%"align="center">
			<tr align="center">
				<td valign="top">
					...
					{
						print "<a href=".$_SERVER['PHP_SELF']."?idnaviera=$row[$llave]>$row[nombre]</a><br>";
					}
					?>
...
<table width="100%" align="center">
    ...
    <tr>
    	<td class="contenidotab">CONTRATO</td>    
    	<td>       
            <table>
                <tr>
                	<td><input id="num_contrato" name="num_contrato" class="tex1" value="<? print $datosad['num_contrato']; ?>" maxlength="50"></td>
                </tr>
            </table>
        </td>       
    </tr>	  
