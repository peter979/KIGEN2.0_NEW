...
if($_POST['datosok']=='si')
{
	...
	elseif(isset($_POST['idpais']) && $_POST['idpais'] != '')
	{
		$queryins="UPDATE $tabla SET nombre=UCASE('$_POST[nombre]'), observaciones=UCASE('$_POST[observaciones]') WHERE $llave='$_POST[idpais]'";
		//print $queryins; 
		$buscarins=mysqli_query($link,$queryins);
...
{
	$queryelim="DELETE FROM $tabla WHERE $llave='$_POST[idpais]'";
...
<input name="idpais" type="hidden" value="<? print $_GET['idpais'] ?>" />
<input name="xyz" type="hidden" value="" />

<?
	if($_GET['idpais'] != '')
	{
		$sqlad = "select * from $tabla where $llave=$_GET[idpais]";
...
if(isset($_POST['xyz']) and $_POST['xyz']!="")
{
	...
	else
	{
		...
		{
			$row = mysqli_fetch_array($query);
			print ("<script>document.location.href='".$_SERVER['PHP_SELF']."?idpais=$row[idpais]'</script>");
		}
		?>
		<table width="50%"align="center">
			<tr align="center">
				<td valign="top">
					...
					{
						print "<a href=".$_SERVER['PHP_SELF']."?idpais=$row[idpais]>$row[nombre]</a><br>";
					}
					?>
...
<? 
	$_POST['idpais'] = $_GET['idpais'];
?>
	<iframe src="ciudades.php?idpais=<? print $_POST['idpais'] ?>" width="100%" height="100%" scrolling="no" frameborder="0"></iframe> 
